...
AsrTrack::AsrTrack (std::string name)
  : Instr (name)
{
}

std::string
AsrTrack::text_get (ProcessorVector* proc)
{
  Fetchqueue queue;
...
void
AsrTrack::exec (ProcessorVector* proc)
{
  Fetchqueue queue;
  queue = proc->queue_get ();
  Quartet q_rx = queue.quartet_get (1);
