...
LcTrack::LcTrack (std::string name)
  : Instr (name)
{
}

std::string
LcTrack::text_get (ProcessorVector* proc)
{
  Fetchqueue queue;
  ...
  std::stringstream ss;
  ss << "lc r" << rx << ", " << std::hex << n_int << std::dec;
...
void
LcTrack::exec (ProcessorVector* proc)
{
  Fetchqueue queue;
  queue = proc->queue_get ();
  Quartet q_rx = queue.quartet_get (2);
