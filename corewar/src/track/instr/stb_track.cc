...
StbTrack::StbTrack (std::string name)
  : Instr (name)
{
}

std::string
StbTrack::text_get (ProcessorVector* proc)
{
  std::stringstream ss;
  ...
  unsigned short jump = q_jump.quartet_get ();
  ...
  unsigned short pos_int = pos;

  ss << "stb [r" << jump << "], " << pos_int << ", " << size_int;
  ...
  void
    StbTrack::exec (ProcessorVector* proc)
  {
    Quartet q_size1 = proc->queue_get ().quartet_get (5);
    Quartet q_size2 = proc->queue_get ().quartet_get (6);
    ...
    else
    {
      ...
      unsigned short jump = proc->registers_get (q_jump.quartet_get ()) %
	Delays::get_instance ().idx_mod_get (proc->ship_type_get ());
      ...
      unsigned short pos_int = (pos + size_int - proc->reexecute_get ());

      proc->track_get ()->write_quartet_buff (proc->buffer_get (pos_int % 64),
					      jump);
    }
  }
